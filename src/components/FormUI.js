	...
	return (
		<form onSubmit={handleSubmit}>
			<input
				type="text"
				minLength="3"
				placeholder={type !== 'earnings' && type !== 'job' && type !== 'roi' ? 'Шығын аты' : 'Кіріс аты'}
				id="name"
			...
			<div>
				<button type="submit">
					...
					<span className="sr-only">Сақтау</span>
				</button>

				<button type="reset" className="delete" onClick={handleReset}> {valueData.id ? 'Болдырмауl' : ' Тазарту'}</button>
			</div>
		</form>
	);
}